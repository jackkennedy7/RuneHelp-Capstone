...
function parseIntent(text) {
    ...
    if (/compare|vs/i.test(text)) return { intent: 'compare' };
...
You have access to live player stats fetched from the OSRS hiscores.
...
async function sendMessage() {
    ...
    try {
        ...
        if (intent === 'lookup' && username) {
            ...
            currentPlayerData = await res.json();
        ...
        typingMsg.textContent = err.message.startsWith('Player not found')
            ? `Couldn't find that player. Check the username and try again.`
